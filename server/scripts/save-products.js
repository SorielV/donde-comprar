...
const files = (readdirSync(path.resolve(process.env.DATA_FOLDER || './data/'))).filter(file => file.endsWith('.json'))
  ...
  for (const file of files) {
    ...
    for (const item of items) {
      ...
      if (item.category && !categories.includes(slug(item.category))) {
        const category = await Category.find({ slug: slug(item.category) }).lean()
      ...
      } else if (item.category && categoriesReference.includes(slug(item.category))) {
